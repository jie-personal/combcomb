...
axios(config)
  .then(function (response) {
    ...
    for (let index = 0; index < classes.length; index++) {
      ...
      $('.main').append(`
                <div class="${className}">
                    <pre><span class="course_name" style="font-family: Arial, Helvetica, sans-serif; font-weight: bold;letter-spacing: 2px">${course.name}</span></pre>
                    <p class="course_date">日期：${course.date}</p>
                    <pre><span class="course_time" style="font-family: Arial, Helvetica, sans-serif; ">時間：${course.start_time} ~ ${course.end_time}</span></pre>
                    <p class="course_desc">${course.description}</p>
